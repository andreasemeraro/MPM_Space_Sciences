...
def ccTOop(R,V,mu):
    ...
    if e > eps:
        ...
        if cp(2) >= 0:
            ...
            TA = 2*np.pi - np.arccos(dot(N,R)/n/r)
    ...
    orbpar=np.array([h,e,RA,incl,w,TA,a,T])

    mod=False

    if mod:
        print("[h(km^2s) e(adim) RA(grad) incl(grad) w(grad) TA(grad) a(km) T(s)]")
        print(f'{h:.1f}',f'{e:.2f}',f'{((180/np.pi)*RA):.2f}',f'{((180/np.pi)*incl):.2f}',f'{((180/np.pi)*w):.2f}',f'{((180/np.pi)*TA):.2f}',f'{a:.1f}',f'{T:.1f}')
    else:
        print("[h(km^2s) e(adim) RA(rad) incl(rad) w(rad) TA(rad) a(km) T(s)]")
        print(f'{h:.1f}',f'{e:.2f}',f'{RA:.2f}',f'{incl:.2f}',f'{w:.2f}',f'{TA:.2f}',f'{a:.1f}',f'{T:.1f}')

    return orbpar

#inserisci qui i valori di R (km), V(km/s) come liste e mu (km^3/s^2)
b=ccTOop([-6045,-3490,2500],[-3.457,6.618,2.533],398600)
d=ccTOop([7000,-12124,0],[2.6679,4.621,0],398600)
#print(b)
